---
id: 0xfac61aeae5776279
title: "Landscape Default Gloss From Spec Mat0 Mat0 Blend World Mask Mat0 Nm Mat0 W Uv B Mat1 Mat1 Blend World Mask Mat1 Nm Mat1 W Uv B Mat2 Mat2 Blend World Mask Mat2 Nm Mat2 W Uv B Mat3 Mat3 Blend World Mask Mat3 Nm Mat3 W Uv B Overlay Over Detail World Mask A Wetness World Nm World Overlay Mask Texture World Overlay Texture"
---

# Landscape Default Gloss From Spec Mat0 Mat0 Blend World Mask Mat0 Nm Mat0 W Uv B Mat1 Mat1 Blend World Mask Mat1 Nm Mat1 W Uv B Mat2 Mat2 Blend World Mask Mat2 Nm Mat2 W Uv B Mat3 Mat3 Blend World Mask Mat3 Nm Mat3 W Uv B Overlay Over Detail World Mask A Wetness World Nm World Overlay Mask Texture World Overlay Texture
(`landscape:default:GLOSS_FROM_SPEC:MAT0:MAT0_BLEND_WORLD_MASK:MAT0_NM:MAT0_W_UV_B:MAT1:MAT1_BLEND_WORLD_MASK:MAT1_NM:MAT1_W_UV_B:MAT2:MAT2_BLEND_WORLD_MASK:MAT2_NM:MAT2_W_UV_B:MAT3:MAT3_BLEND_WORLD_MASK:MAT3_NM:MAT3_W_UV_B:OVERLAY_OVER_DETAIL:WORLD_MASK_A_WETNESS:WORLD_NM:WORLD_OVERLAY_MASK_TEXTURE:WORLD_OVERLAY_TEXTURE`, `0xfac61aeae5776279`)

This template is referenced 1 time by [landscape:GLOSS_FROM_SPEC:MAT0:MAT0_BLEND_WORLD_MASK:MAT0_NM:MAT0_W_UV_B:MAT1:MAT1_BLEND_WORLD_MASK:MAT1_NM:MAT1_W_UV_B:MAT2:MAT2_BLEND_WORLD_MASK:MAT2_NM:MAT2_W_UV_B:MAT3:MAT3_BLEND_WORLD_MASK:MAT3_NM:MAT3_W_UV_B:OVERLAY_OVER_DETAIL:WORLD_MASK_A_WETNESS:WORLD_NM:WORLD_OVERLAY_MASK_TEXTURE:WORLD_OVERLAY_TEXTURE](/docs/Generated/Render%20Templates/base.render_template_database#landscape-gloss-from-spec-mat0-mat0-blend-world-mask-mat0-nm-mat0-w-uv-b-mat1-mat1-blend-world-mask-mat1-nm-mat1-w-uv-b-mat2-mat2-blend-world-mask-mat2-nm-mat2-w-uv-b-mat3-mat3-blend-world-mask-mat3-nm-mat3-w-uv-b-overlay-over-detail-world-mask-a-wetness-world-nm-world-overlay-mask-texture-world-overlay-texture).
This template uses the following inputs:
 |Name|UI Name|Type|
|---|---|---|
|world_overlay_texture|World Overlay Texture|texture|
|world_overlay_mask_texture|World Mask Texture|texture|
|bump_normal_texture|World Normal Map|texture|
|diffuse_layer0_texture|Diffuse Texture Layer 0 [mask.r] (UV2 else UV1)|texture|
|normal_layer0_texture|Normal Map Layer 0 [mask.r] (UV2 else UV1)|texture|
|diffuse_layer1_texture|Diffuse Texture Layer 1 [mask.g] (UV2 else UV1)|texture|
|normal_layer1_texture|Normal Map Layer 1 [mask.g] (UV2 else UV1)|texture|
|diffuse_layer2_texture|Diffuse Texture Layer 2 [mask.b] (UV2 else UV1)|texture|
|normal_layer2_texture|Normal Map Layer 2 [mask.b] (UV2 else UV1)|texture|
|diffuse_layer3_texture|Diffuse Texture Layer 3 [mask.a] (UV2 else UV1)|texture|
|normal_layer3_texture|Normal Map Layer 3 [mask.a] (UV2 else UV1)|texture|
|detail_fadeout|Detail Fade Out (start, end)|vector3|
|detail_scale|Detail Texture Scale A and B|vector3|.

### Technique `deferred_ambient`
### Render States
More info can be found on the [Direct3D Docs](https://learn.microsoft.com/en-us/windows/win32/direct3d9/d3drenderstatetype)
```cpp
enum D3DRENDERSTATETYPE {
	D3DRS_ZENABLE = 1,
	D3DRS_ZFUNC = 4,
	D3DRS_ZWRITEENABLE = 1,
	D3DRS_ALPHABLENDENABLE = 0,
	D3DRS_CULLMODE = 2,
	D3DRS_COLORWRITEENABLE = 15,
	D3DRS_COLORWRITEENABLE1 = 15,
	D3DRS_COLORWRITEENABLE2 = 15,
	D3DRS_STENCILENABLE = 0,
	D3DRS_STENCILFUNC = 1,
	D3DRS_STENCILFAIL = 1,
	D3DRS_STENCILMASK = 0,
	D3DRS_STENCILPASS = 1,
	D3DRS_STENCILREF = 0,
	D3DRS_STENCILWRITEMASK = 0,
	D3DRS_STENCILZFAIL = 1,
	D3DRS_DEPTHBIAS = 0,
	D3DRS_SLOPESCALEDEPTHBIAS = 0,
	D3DRS_FOGENABLE = 0,
	D3DRS_SPECULARENABLE = 0,
	D3DRS_ADAPTIVETESS_X = 0,
	D3DRS_ADAPTIVETESS_Z = 0,
	D3DRS_ADAPTIVETESS_W = 0,
	D3DRS_SRGBWRITEENABLE = 0,
	D3DRS_SEPARATEALPHABLENDENABLE = 0,
	D3DRS_ALPHATESTENABLE = 0,
};
```
### Sampler States
More info can be found on the [Direct3D Docs](https://learn.microsoft.com/en-us/windows/win32/direct3d9/d3dsamplerstatetype)
#### Sampler 0
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 1
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 2
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 0,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 3
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 4
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 0,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 5
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 6
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 0,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 7
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 8
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 0,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 9
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
#### Sampler 10
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 0,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
### Shaders
#### Vertex Shader
```hlsl
float GLOBAL_fov_ratio : register(vs_3_0, c8);
float camera_near_range : register(vs_3_0, c7);
float3 ref_detail_scale : register(vs_3_0, c9);
column_major float4x4 view_proj_matrix : register(vs_3_0, c0);
column_major float4x4 world_matrix : register(vs_3_0, c4);
struct VertexMain_Input
{
    float4 position : POSITION;
    float4 normal : NORMAL;
    float4 texcoord : TEXCOORD;
    float4 texcoord1 : TEXCOORD1;
    float4 tangent : TANGENT;
    float4 binormal : BINORMAL;
};

struct VertexMain_Output
{
    float4 position : POSITION;
    float2 texcoord7 : TEXCOORD7;
    float4 texcoord : TEXCOORD;
    float4 texcoord1 : TEXCOORD1;
    float3 texcoord2 : TEXCOORD2;
    float3 texcoord3 : TEXCOORD3;
    float3 texcoord4 : TEXCOORD4;
    float4 texcoord5 : TEXCOORD5;
};

VertexMain_Output VertexMain(VertexMain_Input i)
{
    VertexMain_Output o;
    float4 temp0;
    float3 temp1;
    // def c10, 2, -1, 0.5, 51200
    // def c11, 9.765625E-06, -9.765625E-06, 0, 0
    // dcl_position v0
    // dcl_normal v1
    // dcl_texcoord v2
    // dcl_texcoord1 v3
    // dcl_tangent v4
    // dcl_binormal v5
    // dcl_position o0
    // dcl_texcoord7 o1.xy
    // dcl_texcoord o2
    // dcl_texcoord1 o3
    // dcl_texcoord2 o4.xyz
    // dcl_texcoord3 o5.xyz
    // dcl_texcoord4 o6.xyz
    // dcl_texcoord5 o7
    // dp4 o0.x, v0, c0
    o.position.x = dot(i.position, (view_proj_matrix._m00_m10_m20_m30));
    // dp4 o0.y, v0, c1
    o.position.y = dot(i.position, (view_proj_matrix._m01_m11_m21_m31));
    // dp4 o0.w, v0, c3
    o.position.w = dot(i.position, (view_proj_matrix._m03_m13_m23_m33));
    // rsq r0.x, c8.x
    temp0.x = 1 / sqrt(GLOBAL_fov_ratio.x);
    // dp4 r0.y, v0, c2
    temp0.y = dot(i.position, (view_proj_matrix._m02_m12_m22_m32));
    // mad o3.w, r0.y, r0.x, c7.x
    o.texcoord1.w = temp0.y * temp0.x + camera_near_range.x;
    // mov o0.z, r0.y
    o.position.z = temp0.y;
    // mad r0.xyz, v4, c10.x, c10.y
    temp0.xyz = i.tangent.xyz * float3(2, 2, 2) + float3(-1, -1, -1);
    // dp3 o5.x, r0, c5
    o.texcoord3.x = dot(temp0.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 o6.x, r0, c6
    o.texcoord4.x = dot(temp0.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // mad r0.xyz, v5, c10.x, c10.y
    temp0.xyz = i.binormal.xyz * float3(2, 2, 2) + float3(-1, -1, -1);
    // dp3 o5.y, -r0, c5
    o.texcoord3.y = dot(-temp0.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 o6.y, -r0, c6
    o.texcoord4.y = dot(-temp0.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // mad r0.xyz, v1, c10.x, c10.y
    temp0.xyz = i.normal.xyz * float3(2, 2, 2) + float3(-1, -1, -1);
    // dp3 r1.x, r0, c4
    temp1.x = dot(temp0.xyz, (world_matrix._m00_m10_m20_m30).xyz);
    // dp3 r1.y, r0, c5
    temp1.y = dot(temp0.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 r1.z, r0, c6
    temp1.z = dot(temp0.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // mad o4.xyz, r1, c10.z, c10.z
    o.texcoord2 = temp1 * float4(0.5, 0.5, 0.5, 0.5) + float4(0.5, 0.5, 0.5, 0.5);
    // mov o5.z, r1.y
    o.texcoord3.z = temp1.y;
    // mov o6.z, r1.z
    o.texcoord4.z = temp1.z;
    // dp4 o3.z, v0, c6
    o.texcoord1.z = dot(i.position, (world_matrix._m02_m12_m22_m32));
    // dp4 r0.x, v0, c4
    temp0.x = dot(i.position, (world_matrix._m00_m10_m20_m30));
    // dp4 r0.y, v0, c5
    temp0.y = dot(i.position, (world_matrix._m01_m11_m21_m31));
    // add r0.zw, r0.xyxy, c10.w
    temp0.zw = temp0.xy + float2(51200, 51200);
    // mov o3.xy, r0
    o.texcoord1.xy = temp0.xy;
    // mul r0.xy, r0.zwzw, c11
    temp0.xy = temp0.zw * float2(9.765625E-06, -9.765625E-06);
    // mul o7, r0.xyxy, c9.xxyy
    o.texcoord5 = temp0.xyxy * ref_detail_scale.xxyy;
    // mov o1.xy, r0
    o.texcoord7 = temp0;
    // mov o2.xy, v2
    o.texcoord.xy = i.texcoord.xy;
    // mov o2.zw, v3.xyxy
    o.texcoord.zw = i.texcoord1.xy;
    // 

    return o;
}
```
Decompiled by DXDecompiler.
#### Pixel Shader
```hlsl
sampler2D bump_normal_texture : register(ps_3_0, s2);
sampler2D diffuse_layer0_texture : register(ps_3_0, s3);
sampler2D diffuse_layer1_texture : register(ps_3_0, s5);
sampler2D diffuse_layer2_texture : register(ps_3_0, s7);
sampler2D diffuse_layer3_texture : register(ps_3_0, s9);
sampler2D normal_layer0_texture : register(ps_3_0, s4);
sampler2D normal_layer1_texture : register(ps_3_0, s6);
sampler2D normal_layer2_texture : register(ps_3_0, s8);
sampler2D normal_layer3_texture : register(ps_3_0, s10);
float3 ref_detail_fadeout : register(ps_3_0, c0);
sampler2D world_overlay_mask_texture : register(ps_3_0, s1);
sampler2D world_overlay_texture : register(ps_3_0, s0);
struct PixelMain_Input
{
    float2 texcoord7 : TEXCOORD7;
    float4 texcoord1 : TEXCOORD1;
    float4 texcoord5 : TEXCOORD5;
};

struct PixelMain_Output
{
    float4 color : COLOR;
    float4 color1 : COLOR1;
    float4 color2 : COLOR2;
};

PixelMain_Output PixelMain(PixelMain_Input i)
{
    PixelMain_Output o;
    float4 temp0, temp1, temp2, temp3, temp4, temp5, temp6, temp7, temp8, temp9, temp10, temp11, temp12;
    // def c1, 2.2, -0.5, 1, 2.5
    // def c2, 0.75, -0.1, 1.1111112, 0
    // def c3, -2, 3, 0, 1
    // dcl_texcoord7 v0.xy
    // dcl_texcoord1 v1.w
    // dcl_texcoord5 v2.zw
    // dcl_2d s0
    // dcl_2d s1
    // dcl_2d s2
    // dcl_2d s3
    // dcl_2d s4
    // dcl_2d s5
    // dcl_2d s6
    // dcl_2d s7
    // dcl_2d s8
    // dcl_2d s9
    // dcl_2d s10
    // texld_pp r0, v0, s0
    temp0 = /* not implemented _pp modifier */ tex2D(world_overlay_texture, i.texcoord7.xy);
    // log r1.x, r0.x
    temp1.x = log2(temp0.x);
    // log r1.y, r0.y
    temp1.y = log2(temp0.y);
    // log r1.z, r0.z
    temp1.z = log2(temp0.z);
    // log r1.w, r0.w
    temp1.w = log2(temp0.w);
    // mul r1, r1, c1.x
    temp1 = temp1 * float4(2.2, 2.2, 2.2, 2.2);
    // exp_pp r2.x, r1.x
    temp2.x = /* not implemented _pp modifier */ exp2(temp1.x);
    // exp_pp r2.y, r1.y
    temp2.y = /* not implemented _pp modifier */ exp2(temp1.y);
    // exp_pp r2.z, r1.z
    temp2.z = /* not implemented _pp modifier */ exp2(temp1.z);
    // exp_pp r2.w, r1.w
    temp2.w = /* not implemented _pp modifier */ exp2(temp1.w);
    // texld_pp r1, v0, s2
    temp1 = /* not implemented _pp modifier */ tex2D(bump_normal_texture, i.texcoord7.xy);
    // add_pp r1.xy, r1.wyzw, c1.y
    temp1.xy = /* not implemented _pp modifier */ temp1.wy + float2(-0.5, -0.5);
    // add_pp r1.xy, r1, r1
    temp1.xy = /* not implemented _pp modifier */ temp1.xy + temp1.xy;
    // mul_pp r1.w, r1.x, r1.x
    temp1.w = /* not implemented _pp modifier */ temp1.x * temp1.x;
    // mad_pp r1.w, r1.y, r1.y, -r1.w
    temp1.w = /* not implemented _pp modifier */ temp1.y * temp1.y + -temp1.w;
    // add_pp r1.w, -r1.w, c1.z
    temp1.w = /* not implemented _pp modifier */ -temp1.w + float1(1);
    // rsq_pp r1.w, r1.w
    temp1.w = /* not implemented _pp modifier */ 1 / sqrt(temp1.w);
    // rcp_pp r1.z, r1.w
    temp1.z = /* not implemented _pp modifier */ 1.0f / temp1.w;
    // if_lt v1.w, c0.y
    if (i.texcoord1.w < ref_detail_fadeout.y) {
        // add r1.w, -c0.x, v1.w
        temp1.w = -ref_detail_fadeout.x + i.texcoord1.w;
        // add r3.x, -c0.x, c0.y
        temp3.x = -ref_detail_fadeout.x + ref_detail_fadeout.y;
        // rcp r3.x, r3.x
        temp3.x = 1.0f / temp3.x;
        // mad_sat_pp r3.x, r1.w, -r3.x, c1.z
        ;// error
        // cmp_pp r1.w, r1.w, r3.x, c1.z
        temp1.w = /* not implemented _pp modifier */ (temp1.w >= 0) ? temp3.x : float1(1);
        // texld_pp r4, v0, s1
        temp4 = /* not implemented _pp modifier */ tex2D(world_overlay_mask_texture, i.texcoord7.xy);
        // texld_pp r5, v2.zwzw, s3
        temp5 = /* not implemented _pp modifier */ tex2D(diffuse_layer0_texture, i.texcoord5.zw);
        // texld_pp r6, v2.zwzw, s5
        temp6 = /* not implemented _pp modifier */ tex2D(diffuse_layer1_texture, i.texcoord5.zw);
        // texld_pp r7, v2.zwzw, s7
        temp7 = /* not implemented _pp modifier */ tex2D(diffuse_layer2_texture, i.texcoord5.zw);
        // texld_pp r8, v2.zwzw, s9
        temp8 = /* not implemented _pp modifier */ tex2D(diffuse_layer3_texture, i.texcoord5.zw);
        // texld_pp r9, v2.zwzw, s4
        temp9 = /* not implemented _pp modifier */ tex2D(normal_layer0_texture, i.texcoord5.zw);
        // texld_pp r10, v2.zwzw, s6
        temp10 = /* not implemented _pp modifier */ tex2D(normal_layer1_texture, i.texcoord5.zw);
        // texld_pp r11, v2.zwzw, s8
        temp11 = /* not implemented _pp modifier */ tex2D(normal_layer2_texture, i.texcoord5.zw);
        // texld_pp r12, v2.zwzw, s10
        temp12 = /* not implemented _pp modifier */ tex2D(normal_layer3_texture, i.texcoord5.zw);
        // lrp_pp r3.yzw, r4.x, r6.xxyz, r5.xxyz
        temp3.yzw = /* not implemented _pp modifier */ lerp(temp5.xyz, temp6.xyz, temp4.xxx);
        // lrp_pp r5.xy, r4.x, r10.wyzw, r9.wyzw
        temp5.xy = /* not implemented _pp modifier */ lerp(temp9.wy, temp10.wy, temp4.xx);
        // lrp_pp r6.xyz, r4.y, r7, r3.yzww
        temp6.xyz = /* not implemented _pp modifier */ lerp(temp3.yzw, temp7.xyz, temp4.yyy);
        // lrp_pp r3.yz, r4.y, r11.xwyw, r5.xxyw
        temp3.yz = /* not implemented _pp modifier */ lerp(temp5.xy, temp11.wy, temp4.yy);
        // lrp_pp r5.xyz, r4.z, r8, r6
        temp5.xyz = /* not implemented _pp modifier */ lerp(temp6.xyz, temp8.xyz, temp4.zzz);
        // lrp_pp r6.xy, r4.z, r12.wyzw, r3.yzzw
        temp6.xy = /* not implemented _pp modifier */ lerp(temp3.yz, temp12.wy, temp4.zz);
        // mul_pp r0.xyz, r0, r5
        temp0.xyz = /* not implemented _pp modifier */ temp0.xyz * temp5.xyz;
        // mad r0.xyz, r0, c1.w, -r2
        temp0.xyz = temp0.xyz * float3(2.5, 2.5, 2.5) + -temp2.xyz;
        // mad_pp oC0.xyz, r3.x, r0, r2
        o.color.xyz = /* not implemented _pp modifier */ temp3.xxx * temp0.xyz + temp2.xyz;
        // add_pp r0.xy, r6, c1.y
        temp0.xy = /* not implemented _pp modifier */ temp6.xy + float2(-0.5, -0.5);
        // add_pp r0.xy, r0, r0
        temp0.xy = /* not implemented _pp modifier */ temp0.xy + temp0.xy;
        // mul_pp r3.x, r0.x, r0.x
        temp3.x = /* not implemented _pp modifier */ temp0.x * temp0.x;
        // mad_pp r3.x, r0.y, r0.y, -r3.x
        temp3.x = /* not implemented _pp modifier */ temp0.y * temp0.y + -temp3.x;
        // add_pp r3.x, -r3.x, c1.z
        temp3.x = /* not implemented _pp modifier */ -temp3.x + float1(1);
        // rsq_pp r3.x, r3.x
        temp3.x = /* not implemented _pp modifier */ 1 / sqrt(temp3.x);
        // rcp_pp r0.z, r3.x
        temp0.z = /* not implemented _pp modifier */ 1.0f / temp3.x;
        // lrp_pp r3.xyz, r1.w, r0, r1
        temp3.xyz = /* not implemented _pp modifier */ lerp(temp1.xyz, temp0.xyz, temp1.www);
        // add_sat r0.x, r2.w, r4.w
        temp0.x = saturate(temp2.w + temp4.w);
        // mul_pp oC0.w, r0.x, c2.x
        o.color.w = /* not implemented _pp modifier */ temp0.x * float1(0.75);
        // add r0.x, r4.w, c2.y
        temp0.x = temp4.w + float1(-0.1);
        // mul_sat r0.x, r0.x, c2.z
        temp0.x = saturate(temp0.x * float1(1.1111112));
        // mad r0.y, r0.x, c3.x, c3.y
        temp0.y = temp0.x * float1(-2) + float1(3);
        // mul r0.x, r0.x, r0.x
        temp0.x = temp0.x * temp0.x;
        // mul r0.x, r0.x, r0.y
        temp0.x = temp0.x * temp0.y;
        // add_sat_pp oC1.w, r0.w, r4.w
        ;// error
        // lrp_pp r1.xyz, r0.x, c3.zzww, r3
        temp1.xyz = /* not implemented _pp modifier */ lerp(temp3.xyz, float3(0, 0, 1), temp0.xxx);
        // else
    } else {
        // mov_pp oC0, r2
        o.color = /* not implemented _pp modifier */ temp2;
        // mov_pp oC1.w, r0.w
        o.color1.w = /* not implemented _pp modifier */ temp0.w;
        // endif
    }
    // mad_pp oC1.xyz, r1, -c1.y, -c1.y
    o.color1.xyz = /* not implemented _pp modifier */ temp1.xyz * float3(0.5, 0.5, 0.5) + float3(0.5, 0.5, 0.5);
    // mul oC2, c3.wzzz, v1.w
    o.color2 = float4(1, 0, 0, 0) * i.texcoord1.w;
    // 

    return o;
}
```
Decompiled by DXDecompiler.
