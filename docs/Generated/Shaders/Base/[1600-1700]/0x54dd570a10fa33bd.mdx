---
id: 0x54dd570a10fa33bd
title: "Generic Default Align Normal World Z Alpha Masked Diffuse Texture Double Sided External Alpha Ref Foliage Rendering Normalmap Vegetation Bending"
---

# Generic Default Align Normal World Z Alpha Masked Diffuse Texture Double Sided External Alpha Ref Foliage Rendering Normalmap Vegetation Bending
(`generic:default:ALIGN_NORMAL_WORLD_Z:ALPHA_MASKED:DIFFUSE_TEXTURE:DOUBLE_SIDED:EXTERNAL_ALPHA_REF:FOLIAGE_RENDERING:NORMALMAP:VEGETATION_BENDING`, `0x54dd570a10fa33bd`)

This template is referenced 1 time by [generic:ALIGN_NORMAL_WORLD_Z:ALPHA_MASKED:DIFFUSE_TEXTURE:DOUBLE_SIDED:EXTERNAL_ALPHA_REF:FOLIAGE_RENDERING:NORMALMAP:VEGETATION_BENDING](/docs/Generated/Render%20Templates/base.render_template_database#generic-align-normal-world-z-alpha-masked-diffuse-texture-double-sided-external-alpha-ref-foliage-rendering-normalmap-vegetation-bending).
This template uses the following inputs:
 |Name|UI Name|Type|
|---|---|---|
|diffuse_texture|Diffuse Texture (UV1)|texture|
|bump_normal_texture|Normal map (UV2 else UV1)|texture|
|speed|Speed|scalar|
|detail_settings|Detail Bending Amplitude / Frequency|vector3|
|branch_settings|Branch Bending Amplitude / Frequency|vector3|
|alpha_ref|Alpha Mask Reference|scalar|.

### Technique `deferred_ambient_alpha_masked`
### Render States
More info can be found on the [Direct3D Docs](https://learn.microsoft.com/en-us/windows/win32/direct3d9/d3drenderstatetype)
```cpp
enum D3DRENDERSTATETYPE {
	D3DRS_ZENABLE = 1,
	D3DRS_ZFUNC = 4,
	D3DRS_ZWRITEENABLE = 1,
	D3DRS_ALPHABLENDENABLE = 0,
	D3DRS_CULLMODE = 1,
	D3DRS_COLORWRITEENABLE = 15,
	D3DRS_COLORWRITEENABLE1 = 15,
	D3DRS_COLORWRITEENABLE2 = 15,
	D3DRS_STENCILENABLE = 0,
	D3DRS_STENCILFUNC = 1,
	D3DRS_STENCILFAIL = 1,
	D3DRS_STENCILMASK = 0,
	D3DRS_STENCILPASS = 1,
	D3DRS_STENCILREF = 0,
	D3DRS_STENCILWRITEMASK = 0,
	D3DRS_STENCILZFAIL = 1,
	D3DRS_DEPTHBIAS = 0,
	D3DRS_SLOPESCALEDEPTHBIAS = 0,
	D3DRS_FOGENABLE = 0,
	D3DRS_SPECULARENABLE = 0,
	D3DRS_ADAPTIVETESS_X = 0,
	D3DRS_ADAPTIVETESS_Z = 0,
	D3DRS_ADAPTIVETESS_W = 0,
	D3DRS_SRGBWRITEENABLE = 0,
	D3DRS_SEPARATEALPHABLENDENABLE = 0,
	D3DRS_ALPHATESTENABLE = 0,
};
```
### Sampler States
More info can be found on the [Direct3D Docs](https://learn.microsoft.com/en-us/windows/win32/direct3d9/d3dsamplerstatetype)
#### Sampler 0
```cpp
enum D3DSAMPLERSTATETYPE {
	D3DSAMP_MIPMAPLODBIAS = 827606343,
	D3DSAMP_SRGBTEXTURE = 1,
	D3DSAMP_ADDRESSU = 1,
	D3DSAMP_ADDRESSV = 1,
	D3DSAMP_MAGFILTER = 3,
	D3DSAMP_MINFILTER = 3,
	D3DSAMP_MIPFILTER = 3,
};
```
### Shaders
#### Vertex Shader
```hlsl
float camera_near_range : register(vs_3_0, c12);
column_major float4x4 proj_matrix : register(vs_3_0, c8);
float3 ref_branch_settings : register(vs_3_0, c16);
float3 ref_detail_settings : register(vs_3_0, c15);
float ref_speed : register(vs_3_0, c14);
float ref_time : register(vs_3_0, c13);
column_major float4x4 view_matrix : register(vs_3_0, c0);
column_major float4x4 world_matrix : register(vs_3_0, c4);
struct VertexMain_Input
{
    float4 position : POSITION;
    float4 texcoord1 : TEXCOORD1;
    float4 texcoord : TEXCOORD;
    float4 tangent : TANGENT;
    float4 binormal : BINORMAL;
    float4 color : COLOR;
};

struct VertexMain_Output
{
    float4 position : POSITION;
    float texcoord1 : TEXCOORD1;
    float4 texcoord : TEXCOORD;
    float3 texcoord2 : TEXCOORD2;
    float3 texcoord3 : TEXCOORD3;
    float3 texcoord4 : TEXCOORD4;
    float3 texcoord5 : TEXCOORD5;
    float4 color : COLOR;
};

VertexMain_Output VertexMain(VertexMain_Input i)
{
    VertexMain_Output o;
    float4 temp0, temp1, temp2, temp3;
    // def c17, 2000, -0.5, 2, -1
    // def c18, 1.975, 0.793, 0.375, 0.193
    // def c19, 2, 3, 0, 0
    // dcl_position v0
    // dcl_texcoord1 v1
    // dcl_texcoord v2
    // dcl_tangent v3
    // dcl_binormal v4
    // dcl_color v5
    // dcl_position o0
    // dcl_texcoord1 o1.x
    // dcl_texcoord o2
    // dcl_texcoord2 o3.xyz
    // dcl_texcoord3 o4.xyz
    // dcl_texcoord4 o5.xyz
    // dcl_texcoord5 o6.xyz
    // dcl_color o7
    // mov r0.x, c4.w
    temp0.x = (world_matrix._m00_m10_m20_m30).w;
    // mov r0.y, c5.w
    temp0.y = (world_matrix._m01_m11_m21_m31).w;
    // mov r0.z, c6.w
    temp0.z = (world_matrix._m02_m12_m22_m32).w;
    // dp3 r0.x, r0, -c17.w
    temp0.x = dot(temp0.xyz, float3(1, 1, 1));
    // add r0.x, r0.x, v5.y
    temp0.x = temp0.x + i.color.y;
    // add r0.x, r0.x, c13.x
    temp0.x = temp0.x + ref_time.x;
    // mul r0.zw, r0.x, c16.y
    temp0.zw = temp0.x * ref_branch_settings.y;
    // add r1.x, c13.x, v5.y
    temp1.x = ref_time.x + i.color.y;
    // mul r0.xy, r1.x, c15.y
    temp0.xy = temp1.x * ref_detail_settings.y;
    // mul r0, r0, c14.x
    temp0 = temp0 * ref_speed.x;
    // mul r0, r0, c18
    temp0 = temp0 * float4(1.975, 0.793, 0.375, 0.193);
    // frc r0, r0
    temp0 = frac(temp0);
    // mad r0, r0, c17.z, c17.y
    temp0 = temp0 * float4(2, 2, 2, 2) + float4(-0.5, -0.5, -0.5, -0.5);
    // frc r0, r0
    temp0 = frac(temp0);
    // mad r0, r0, c17.z, c17.w
    temp0 = temp0 * float4(2, 2, 2, 2) + float4(-1, -1, -1, -1);
    // mul r1, r0_abs, r0_abs
    temp1 = abs(temp0) * abs(temp0);
    // mad r0, r0_abs, -c19.x, c19.y
    temp0 = abs(temp0) * float4(-2, -2, -2, -2) + float4(3, 3, 3, 3);
    // mul r0, r0, r1
    temp0 = temp0 * temp1;
    // add r0.xyz, r0.yyww, r0.xxzw
    temp0.xyz = temp0.yyw + temp0.xxz;
    // mul r0.w, v5.z, v5.x
    temp0.w = i.color.z * i.color.x;
    // mul r0.w, r0.w, c15.x
    temp0.w = temp0.w * ref_detail_settings.x;
    // add r1.xyz, c17.y, v5
    temp1.xyz = float3(-0.5, -0.5, -0.5) + i.color.xyz;
    // mul r1.w, c17.x, v5.w
    temp1.w = float1(2000) * i.color.w;
    // mul r1.xyz, r1.w, r1
    temp1.xyz = temp1.www * temp1.xyz;
    // rcp r1.w, r1.w
    temp1.w = 1.0f / temp1.w;
    // mad r1.xyz, r1, -c17.z, v0
    temp1.xyz = temp1.xyz * float3(-2, -2, -2) + i.position.xyz;
    // dp3 r2.x, r1, r1
    temp2.x = dot(temp1.xyz, temp1.xyz);
    // rsq r2.x, r2.x
    temp2.x = 1 / sqrt(temp2.x);
    // mul r1.xyz, r1, r2.x
    temp1.xyz = temp1.xyz * temp2.xxx;
    // rcp r2.x, r2.x
    temp2.x = 1.0f / temp2.x;
    // mul r1.w, r1.w, r2.x
    temp1.w = temp1.w * temp2.x;
    // dp3 r2.z, r1, c4
    temp2.z = dot(temp1.xyz, (world_matrix._m00_m10_m20_m30).xyz);
    // dp3 r2.w, r1, c5
    temp2.w = dot(temp1.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 o5.z, r1, c6
    o.texcoord4.z = dot(temp1.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // mul r1.xy, r0.w, r2.zwzw
    temp1.xy = temp0.ww * temp2.zw;
    // mov o4.z, r2.w
    o.texcoord3.z = temp2.w;
    // dp4 r3.x, v0, c4
    temp3.x = dot(i.position, (world_matrix._m00_m10_m20_m30));
    // dp4 r3.y, v0, c5
    temp3.y = dot(i.position, (world_matrix._m01_m11_m21_m31));
    // dp4 r3.z, v0, c6
    temp3.z = dot(i.position, (world_matrix._m02_m12_m22_m32));
    // mul r1.z, c16.x, v5.z
    temp1.z = ref_branch_settings.x * i.color.z;
    // mad r0.xyz, r0, r1, r3
    temp0.xyz = temp0.xyz * temp1.xyz + temp3.xyz;
    // dp4 r0.w, v0, c7
    temp0.w = dot(i.position, (world_matrix._m03_m13_m23_m33));
    // dp4 r3.x, r0, c0
    temp3.x = dot(temp0, (view_matrix._m00_m10_m20_m30));
    // dp4 r3.y, r0, c1
    temp3.y = dot(temp0, (view_matrix._m01_m11_m21_m31));
    // dp4 r3.z, r0, c2
    temp3.z = dot(temp0, (view_matrix._m02_m12_m22_m32));
    // dp4 r3.w, r0, c3
    temp3.w = dot(temp0, (view_matrix._m03_m13_m23_m33));
    // mov o6.xyz, r0
    o.texcoord5 = temp0;
    // dp4 o0.x, r3, c8
    o.position.x = dot(temp3, (proj_matrix._m00_m10_m20_m30));
    // dp4 o0.y, r3, c9
    o.position.y = dot(temp3, (proj_matrix._m01_m11_m21_m31));
    // dp4 o0.w, r3, c11
    o.position.w = dot(temp3, (proj_matrix._m03_m13_m23_m33));
    // dp4 r0.x, r3, c10
    temp0.x = dot(temp3, (proj_matrix._m02_m12_m22_m32));
    // add o1.x, r0.x, c12.x
    o.texcoord1 = temp0.x + camera_near_range.x;
    // mov o0.z, r0.x
    o.position.z = temp0.x;
    // mad r0.xyz, v3, c17.z, c17.w
    temp0.xyz = i.tangent.xyz * float3(2, 2, 2) + float3(-1, -1, -1);
    // dp3 o4.x, r0, c5
    o.texcoord3.x = dot(temp0.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 o5.x, r0, c6
    o.texcoord4.x = dot(temp0.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // dp3 r2.x, r0, c4
    temp2.x = dot(temp0.xyz, (world_matrix._m00_m10_m20_m30).xyz);
    // mad r0.xyz, v4, c17.z, c17.w
    temp0.xyz = i.binormal.xyz * float3(2, 2, 2) + float3(-1, -1, -1);
    // dp3 o4.y, -r0, c5
    o.texcoord3.y = dot(-temp0.xyz, (world_matrix._m01_m11_m21_m31).xyz);
    // dp3 o5.y, -r0, c6
    o.texcoord4.y = dot(-temp0.xyz, (world_matrix._m02_m12_m22_m32).xyz);
    // dp3 r2.y, -r0, c4
    temp2.y = dot(-temp0.xyz, (world_matrix._m00_m10_m20_m30).xyz);
    // mov o3.xyz, r2
    o.texcoord2 = temp2;
    // rsq r0.x, r1.w
    temp0.x = 1 / sqrt(temp1.w);
    // mov o7.w, r1.w
    o.color.w = temp1.w;
    // rcp o7.xyz, r0.x
    o.color.xyz = 1.0f / temp0.x;
    // mov o2.xy, v2
    o.texcoord.xy = i.texcoord.xy;
    // mov o2.zw, v1.xyxy
    o.texcoord.zw = i.texcoord1.xy;
    // 

    return o;
}
```
Decompiled by DXDecompiler.
#### Pixel Shader
```hlsl
sampler2D diffuse_texture : register(ps_3_0, s0);
float ref_alpha_ref : register(ps_3_0, c0);
struct PixelMain_Input
{
    float texcoord1 : TEXCOORD1;
    float2 texcoord : TEXCOORD;
    float3 color : COLOR;
};

struct PixelMain_Output
{
    float4 color : COLOR;
    float4 color1 : COLOR1;
    float4 color2 : COLOR2;
};

PixelMain_Output PixelMain(PixelMain_Input i)
{
    PixelMain_Output o;
    float4 temp0, temp1;
    // def c1, 1, -1, 0.4, 0
    // def c2, 0, 1, 0.5, 0
    // dcl_texcoord1 v0.x
    // dcl_texcoord v1.xy
    // dcl_color v2.xyz
    // dcl_2d s0
    // texld_pp r0, v1, s0
    temp0 = /* not implemented _pp modifier */ tex2D(diffuse_texture, i.texcoord.xy);
    // add r1.x, r0.w, -c0.x
    temp1.x = temp0.w + -ref_alpha_ref.x;
    // cmp r1, r1.x, c1.x, c1.y
    temp1 = (temp1.x >= 0) ? float4(1, 1, 1, 1) : float4(-1, -1, -1, -1);
    // texkill r1
    clip(temp1);
    // mul_pp oC0.xyz, r0, v2
    o.color.xyz = /* not implemented _pp modifier */ temp0.xyz * i.color.xyz;
    // mad_pp oC1, r0.w, c2.xxxy, c2.zzyx
    o.color1 = /* not implemented _pp modifier */ temp0.w * float4(0, 0, 0, 1) + float4(0.5, 0.5, 1, 0);
    // mov oC0.w, c1.z
    o.color.w = float1(0.4);
    // mov oC2, v0.x
    o.color2 = i.texcoord1.x;
    // 

    return o;
}
```
Decompiled by DXDecompiler.
